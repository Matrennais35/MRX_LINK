...
QUESTIONS = [
    # — the 10-question battery (same as eval_run) —
...
lines = [f"# Designer review — {len(QUESTIONS)} blueprints ({stamp})",
...
for n, question in enumerate(QUESTIONS, 1):
    ...
    t0 = time.monotonic()
    try:
        blueprint = designer.design(llm, question)
        seconds = time.monotonic() - t0
        lines += [f"\n---\n\n## Q{n} ({seconds:.0f}s): {question}", "",
