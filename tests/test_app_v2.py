"""End-to-end tests for app_v2.py's chat UI, using Streamlit's AppTest to run
the real script. Mirrors test_app.py's approach; the one V2-specific wrinkle
is the fake LLM must also answer the per-step StepDecision call.
"""
...
def test_investigation_trace_is_shown_with_the_step_reasoning():
    ...
    assert not at.exception
    # The trace expander renders each step's reasoning — the V2 headline view.
    all_markdown = " ".join(
